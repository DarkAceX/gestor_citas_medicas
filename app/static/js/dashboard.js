...
function closeModalCitasPendientes() {
    ...
    document.getElementById("citas_pendientes_container").innerHTML = "<p>Cargando citas...</p>";
    deshabilitarBotonesAccionCita();
}
...
function closeModalHistorialCitas() {
    const modal = document.getElementById("modal_historialCitas");
    modal.classList.add("hidden");
    document.getElementById("historial_citas_container").innerHTML = "<p>Cargando historial...</p>"; // Resetear al cerrar
}
...
window.addEventListener("click", function (e) {
    ...
    if (e.target === modalHistorial) { // Nuevo
...
async function cargarDoctores(idEspecialidad) {
    ...
    if (idEspecialidad) {
        ...
        try {
            const response = await fetch(`/doctores/${idEspecialidad}`);
            ...
            if (!response.ok) {
                ...
                console.error(`Error HTTP al cargar doctores: ${response.status} - ${errorText}`);
                alert(`Error al cargar los doctores: ${response.status}. ${errorText}. Por favor, intente de nuevo.`);
            ...
            if (doctores.length === 0) {
                ...
                console.warn(`No se encontraron doctores para la especialidad ${idEspecialidad}.`);
                return;
            }
        ...
        console.log('No se seleccionó una especialidad, reseteando lista de doctores.');
    }
}
...
function handleCitaSelection() {
    ...
    if (selectedCount === 1) {
        ...
        document.getElementById("btnPagarCita").onclick = () => modalPago(selectedCitaId, selectedCitaMonto || '0.00');
        document.getElementById("btnCancelarCita").onclick = () => confirmarCancelacion(selectedCitaId);
    } else {
        deshabilitarBotonesAccionCita();
...
async function cargarCitasPendientes() {
    ...
    try {
        const response = await fetch('/citas_pendientes_json');
        if (!response.ok) {
        ...
        if (citas.length === 0) {
            container.innerHTML = "<p>No tienes citas pendientes en este momento.</p>";
            return;
        }
...
async function cargarHistorialCitas() {
    ...
    try {
        const response = await fetch('/historial_citas_json');
        if (!response.ok) {
            const errorText = await response.text();
            throw new Error(`HTTP error! status: ${response.status} - ${errorText}`);
        }
        ...
        if (citas.length === 0) {
            container.innerHTML = "<p>No hay citas en el historial (pagadas o canceladas).</p>";
            return;
        }
    ...
    if (seguro) {
        try {
            ...
            if (result.success) {
                ...
                alert("Error al cancelar la cita: " + (result.message || "Un error desconocido ocurrió."));
            }
        } catch (error) {
        ...
        formAgendar.addEventListener("submit", async function (e) {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    alert("Error del servidor: " + (errorResponse.message || "Ver consola para detalles."));
                    return;
                }
    ...
    if (formPago) {
        formPago.addEventListener("submit", async function (e) {
            ...
            try {
                ...
                if (result.success) {
                    alert("Pago procesado exitosamente. La cita ha sido marcada como pagada.");
                    closeModalPago();
                    closeModalCitasPendientes();
                    cargarCitasPendientes();

                    // Redirigir al usuario a la URL que genera el PDF
                    window.open(`/generar_factura_pdf/${result.id_cita_pagada}`, '_blank'); // Usar el ID devuelto por el servidor

                } else {
                    alert("Error al procesar el pago: " + (result.message || "Un error desconocido ocurrió."));
                }
            } catch (error) {
    ...
    formPago.addEventListener("submit", async function (e) {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Redirigir al usuario a la URL que genera el PDF
                window.open(`/generar_factura_pdf/${result.id_cita_pagada}`, '_blank');
        ...
        formPago.addEventListener("submit", async function (e) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // ¡NUEVO!: Generar y descargar la factura
                    // Redirigir al usuario a la URL que genera el PDF
                    if (result.id_cita_pagada) { // Asegúrate de que el ID de la cita pagada esté presente
                        window.open(`/generar_factura_pdf/${result.id_cita_pagada}`, '_blank');
                    } else {
                        console.warn("No se recibió el ID de la cita pagada para generar la factura.");
                    }
